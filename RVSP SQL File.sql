...
-- AS we know that movie Table contain 7997 rows from above query
-- So directly count the NULL Values
SELECT 
       COUNT(*) - COUNT(id) AS id_Null_count,
       COUNT(*) - COUNT(title) AS title_Null_count,
       COUNT(*) - COUNT(year) AS year_Nill_count,
...
-- Output format for the first part:
-- Yearwise Count OF movies produced
select year, count(id) AS number_of_movies
from movie
GROUP BY year
ORDER BY year;

-- Output format for the second part:
-- Monthwise Count OF movies produced
...
SELECT COUNT(id) AS countrywise_number_of_movies, year
FROM movie
WHERE country = 'USA' OR country = 'India'
GROUP BY country
HAVING year=2019;

-- USA and India produced 1007 movies in year 2019
...
-- So there is 13 genres of movies
...
-- Drama genre has highest of movies which is around 4285
...
(
SELECT movie_id,
      COUNT(genre) as no_movies
	FROM genre
    GROUP BY movie_id
    HAVING no_movies = 1
    )
    
SELECT COUNT(movie_id) AS no_movies 
      FROM genre_summary;

-- There are 3289 movies having 1 genre 
...
-- German Movie count is more than Italian
--  So, Answer is Yes
...
-- Excluding name column other columns have high number of Null values
...
INNER JOIN ratings AS K
ON M.id = K.movie_id 
INNER JOIN role_mapping AS H
ON M.id=H.movie_id 
INNER JOIN names AS P 
ON H.name_id=P.id
